...
2. Treat [docs/functional-scope-realignment.md](/Users/qyx/Desktop/project/thinking-ide/docs/functional-scope-realignment.md) as the corrective baseline whenever local checkpoint work starts competing with still-open MVP feature-range gaps.
3. Treat the older Wave 1 split-pane slices as superseded shell experiments. The active Wave 1 shell baseline is now [docs/sidepanel-first-refactor-checklist.md](/Users/qyx/Desktop/project/thinking-ide/docs/sidepanel-first-refactor-checklist.md).
4. Treat the first `sidePanel-first` runtime checkpoint as landed: browser-owned sidePanel entry, background lifecycle ownership, a lightweight host runtime bridge, and a dedicated panel session controller now form the default Wave 1 runtime path.
5. Keep Wave 1 shell work bounded to obvious sidePanel product gaps; do not let deeper shell polish displace still-open MVP feature-range items from the PRD and MVP docs.
6. Treat the first Wave 2 adapter-identity slice, the first full-session history-input checkpoint, and the later multi-turn generation checkpoints as landed only as partial foundations; the next priority is not more local polish but making long-session input materially satisfy the documented MVP feature range.
7. Treat the answer-structure checkpoints as partially landed: `answer_outline` now follows a payload-markdown-AST-first path with empty-outline fallback and first-pass heading-tree semantics, but paragraph/block anchors and richer section modeling are still open.
8. Treat the frontend rendering direction as reset to `Structure View first`: the next user-facing rendering target is a collapsible `Session -> Question -> Answer -> Outline` tree, while `Concept View` remains a later enhancement path.
9. Treat concept-quality work as foundation for a later `Concept View`, not as the current primary rendering mode.
10. Treat Wave 3 prep as implementation-ready: provider-backed AI structuring with `DeepSeek` in the first candidate batch now has fixtures and a normalized provider-draft contract, but no runtime wiring yet.
11. Prioritize the still-open product-level gaps called out by [docs/functional-scope-realignment.md](/Users/qyx/Desktop/project/thinking-ide/docs/functional-scope-realignment.md), especially full long-session usable input, the structure-first tree view, provider-backed structuring, privacy-boundary correction, and acceptance-grade test coverage.
