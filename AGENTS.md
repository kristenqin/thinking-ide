...
7. Treat the answer-structure and concept-quality checkpoints as landed only as heuristics: `answer_outline` exists, H1-first source anchors exist, and concept titles are shorter, but the current outline path still derives from flattened answer text; richer answer structure, heading-tree-first extraction with empty-outline fallback, short-concept acceptance, and broader graph semantics are still open.
