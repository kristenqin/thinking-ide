...
7. Treat the answer-structure and concept-quality checkpoints as landed only as heuristics: `answer_outline` exists, H1-first source anchors exist, and concept titles are shorter, but the current outline path still derives from flattened answer text; richer answer structure, payload-markdown-AST-first extraction with empty-outline fallback, short-concept acceptance, and broader graph semantics are still open.
...
9. Prioritize the still-open product-level gaps called out by [docs/functional-scope-realignment.md](/Users/qyx/Desktop/project/thinking-ide/docs/functional-scope-realignment.md), especially full long-session usable input, provider-backed structuring, privacy-boundary correction, and acceptance-grade test coverage.
