...
7. Treat the answer-structure and concept-quality checkpoints as partially landed: `answer_outline` now follows a payload-markdown-AST-first path with empty-outline fallback, H1-first source anchors exist, and concept titles are shorter, but richer heading-tree semantics, short-concept acceptance, paragraph/block anchors, and broader graph semantics are still open.
