...
4. Treat the next Wave 1 checkpoint as landed: sidePanel-native copy/callouts and shell cleanup now remove most of the active layout/overlay residue, but shell `acceptance` is still open.
...
9. Continue the remaining Wave 1 parity tail only where it closes the sidePanel-product gap, especially deeper shell productization, richer restored/partial-history/failed state presentation, and removal of transplanted in-page workspace assumptions after the newly landed entry-state honesty checkpoint.
