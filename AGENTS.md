...
7. Treat the answer-structure and concept-quality checkpoints as partially landed: `answer_outline` now follows a payload-markdown-AST-first path with empty-outline fallback and first-pass heading-tree semantics, H1-first source anchors exist, and concept titles are shorter, but paragraph/block anchors, short-concept acceptance, and broader graph semantics are still open.
