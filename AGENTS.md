...
10. Treat the second Wave 2 generation checkpoint as landed: the runtime now materializes multi-turn `question` / `answer` node chains from the full-session history path instead of collapsing graph generation to the latest exchange, while `answer_outline` and richer concept quality remain open.
