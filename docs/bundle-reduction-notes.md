...
3. Treat persistence splitting as a real-browser validation task, not a paper optimization.
   A naive attempt to defer Dexie behind dynamic `import()` reduced bundle size, but it also broke unpacked-extension injection in the real runtime harness. Any future content-script split on the persistence path must be proven by `npm run runtime:validate:built`, not only by `vite build`.
...
5. Any code-splitting change on the content-script path must also pass unpacked-extension runtime validation, because build-only success is not enough for this repo.
