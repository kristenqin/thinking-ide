...
| TR-16 | Handle source-lost / adapter errors / empty and generation states clearly | Interaction spec, component spec, test docs | Store now persists `lost` source status and raises notice feedback when jump-to-source fails; the panel now has stronger workspace empty-state and status surfaces, but source-lost still rides on shared feedback rather than node-level treatment | Partial | State presentation is stronger, but source-lost iconography, tooltip treatment, and richer failure-state coverage are still incomplete |
