...
| TR-11 | Support deleting nodes and edges | PRD, interaction spec, task breakdown phase 5 | `useThinkingStore` now supports logical delete for nodes/edges and latest-removal undo; `ConceptMapCanvas.tsx` exposes delete actions plus Delete/Backspace handling | Partial | Delete and latest undo exist, but the broader toolbar/popover flow is still thinner than the full interaction spec |
| TR-12 | Let users jump from a node back to the original chat source | PRD MVP goals; interaction spec; task breakdown phase 7 | `src/services/sourceLocator.ts` now resolves by role, dom id, occurrence index, and preview slices; `ConceptMapCanvas.tsx` exposes "Jump to source" | Partial | Stronger than the first cut, but still heuristic and missing explicit source-lost UX |
...
| TR-14 | Preserve user edits across future parsing/regeneration | Interaction spec (`用户编辑优先`), data model doc | `src/services/documentBuilder.ts` now merges regenerated documents with prior node ids, positions, titles, preserved settings, and manual `relates` edges | Partial | Safer than full replace, but broader merge behavior is still heuristic |
...
| TR-18 | Enforce repository quality gates for the MVP slice | `docs/definition-of-done.md`, CI workflow | `package.json` defines `check`, `test`, `test:run`, `build`, `verify`, `ci`; `.github/workflows/ci.yml` runs `npm run ci` | Implemented | Quality gate now covers typecheck, focused automated tests, and build |
...
2. The biggest remaining requirement gaps are richer regeneration/merge behavior, explicit delete flows, and stronger source-lost/source-identity handling.
