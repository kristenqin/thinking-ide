...
| GAP-06 | Markdown / answer_outline 仍停留在启发式 checkpoint | 回答结构项应生成 `answer_outline node`，并能定位到回答段落；概念提取应利用标题、段落主题、步骤、观点、结论。见 [thinking_ide_prd_产品需求文档.md](/Users/qyx/Desktop/project/thinking-ide/docs/specs/thinking_ide_prd_产品需求文档.md) | 当前已新增启发式 `answer_outline` 节点和 `contains` 关系；[src/utils/text.ts](/Users/qyx/Desktop/project/thinking-ide/src/utils/text.ts) 会从 Markdown heading / list / paragraph theme 中切出 outline 项，但没有 Markdown AST，也没有段落级 anchor | P0 | 解析对象比最初更接近需求，但仍未达到可精确定位回答段落的 acceptance |
