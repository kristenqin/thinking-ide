...
| GAP-06 | Markdown / answer_outline 仍停留在 heading-tree checkpoint | 回答结构项应生成 `answer_outline node`，并能定位到回答段落；概念提取应利用标题、段落主题、步骤、观点、结论。见 [thinking_ide_prd_产品需求文档.md](/Users/qyx/Desktop/project/thinking-ide/docs/specs/thinking_ide_prd_产品需求文档.md) | 当前 `answer_outline` 已从扁平 `answer.text` 启发式切分迁移到 payload-markdown-AST-first 路径： [src/services/chatAdapter.ts](/Users/qyx/Desktop/project/thinking-ide/src/services/chatAdapter.ts) 会为 payload-backed assistant messages 保留 `markdownText`， [src/utils/markdown.ts](/Users/qyx/Desktop/project/thinking-ide/src/utils/markdown.ts) 负责从 markdown 语法树中提取 heading 和 section 文本， [src/services/generator.ts](/Users/qyx/Desktop/project/thinking-ide/src/services/generator.ts) 则按 heading tree 生成嵌套 `answer_outline`，并在无 heading 时返回空 outline。当前剩余缺口变成段落/区块级 source anchor，以及不只是 heading 的更完整 answer structure | P0 | 结构来源和 heading-tree 语义已经纠正，但 acceptance 仍要求继续走向 section/paragraph 级定位和更丰富的 section 建模 |
