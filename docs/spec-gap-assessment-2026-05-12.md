...
| GAP-06 | Markdown / answer_outline 仍停留在启发式 checkpoint | 回答结构项应生成 `answer_outline node`，并能定位到回答段落；概念提取应利用标题、段落主题、步骤、观点、结论。见 [thinking_ide_prd_产品需求文档.md](/Users/qyx/Desktop/project/thinking-ide/docs/specs/thinking_ide_prd_产品需求文档.md) | 当前已新增启发式 `answer_outline` 节点和 `contains` 关系；[src/utils/text.ts](/Users/qyx/Desktop/project/thinking-ide/src/utils/text.ts) 现在会优先使用 Markdown H1 heading，再回退到 list / paragraph theme；[src/services/sourceLocator.ts](/Users/qyx/Desktop/project/thinking-ide/src/services/sourceLocator.ts) 也会先尝试匹配 assistant reply 内的同名 H1，但没有 Markdown AST，也没有更稳定的段落级 anchor | P0 | 解析对象比最初更接近需求，但仍未达到可精确定位回答段落的 acceptance |
...
| GAP-09 | Source 跳转只覆盖单来源消息级场景 | concept 多来源需要 `SourceListPopover`，answer_outline 需要段落级定位，恢复需要 `textHash + orderIndex` 辅助。见 [thinking_ide_prd_产品需求文档.md](/Users/qyx/Desktop/project/thinking-ide/docs/specs/thinking_ide_prd_产品需求文档.md)、[thinking_ide_组件设计文档.md](/Users/qyx/Desktop/project/thinking-ide/docs/specs/thinking_ide_组件设计文档.md) | [src/services/sourceLocator.ts](/Users/qyx/Desktop/project/thinking-ide/src/services/sourceLocator.ts) 现在会在 assistant reply 内优先尝试匹配同名 H1 heading，但整体 anchor 仍然是消息级 `SourceRef`；没有 `SourceListPopover`，没有段落锚点，也没有多来源概念跳转 | P1 | 原文联动比纯消息块级更接近需求，但仍只覆盖了 MVP 中较窄的一段 |
