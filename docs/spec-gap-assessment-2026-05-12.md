...
| GAP-06 | Markdown / answer_outline 仍停留在启发式 checkpoint | 回答结构项应生成 `answer_outline node`，并能定位到回答段落；概念提取应利用标题、段落主题、步骤、观点、结论。见 [thinking_ide_prd_产品需求文档.md](/Users/qyx/Desktop/project/thinking-ide/docs/specs/thinking_ide_prd_产品需求文档.md) | 当前已新增启发式 `answer_outline` 节点和 `contains` 关系；但生成路径仍主要从 [src/services/generator.ts](/Users/qyx/Desktop/project/thinking-ide/src/services/generator.ts) 里的扁平 `answer.text` 出发，再通过 [src/utils/text.ts](/Users/qyx/Desktop/project/thinking-ide/src/utils/text.ts) 猜测 outline item。[src/models/source.ts](/Users/qyx/Desktop/project/thinking-ide/src/models/source.ts)、[src/services/chatAdapter.ts](/Users/qyx/Desktop/project/thinking-ide/src/services/chatAdapter.ts) 和 [src/services/sourceLocator.ts](/Users/qyx/Desktop/project/thinking-ide/src/services/sourceLocator.ts) 已经支持 durable `heading` source anchor，并在 live assistant DOM 可用时先定位同名 H1，但当前仍存在“在已渲染 HTML 的 `textContent` 上反推 Markdown 结构”的错层；没有 heading 时也还会退回到 list / paragraph / sentence 级猜测，而不是直接返回空 outline | P0 | 当前主要问题不是标题规则太弱，而是解析对象错了；acceptance 需要改成 DOM heading-tree-first，并在无 heading 时返回空 outline |
