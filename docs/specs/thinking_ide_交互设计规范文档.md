...
1. 默认主视图优先是结构树，而不是 graph-first 的 Concept Map Canvas。
2. 主结构为 `Session -> Question -> Answer -> Outline`。
3. 第一优先交互是展开/收起、选中、回到原文和轻量节点操作。
4. `Concept View` 保留为后续增强视图。

## 3. 交互设计原则

### 3.1 结构浏览优先

当前 MVP 优先让用户通过结构树快速理解长会话内容，因此优先级补充为：

```text
结构浏览与展开/收起 > 原文联动 > 轻量直接操作 > 低频菜单
```

### 3.2 直接操作优先

Thinking IDE 仍然不是节点管理后台。当前结构树节点与后续 Concept View 节点都应尽量支持直接操作。
