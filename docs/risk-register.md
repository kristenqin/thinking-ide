...
| R-06 | Runtime test coverage is still narrow | `ci` now includes unpacked-extension runtime validation, but automated coverage still focuses on a mock host plus generator, document merge, and store basics | Regressions in DOM parsing, observation, and source reveal can still slip through before live-ChatGPT validation | Extend tests to `chatAdapter`, source location behavior, richer merge scenarios, and more DOM fixtures as the code is refactored for testability | Engineering | Active |
...
| R-08 | Product-spec drift is growing around interaction details | Current UI now supports regenerate, drag/connect, rename-on-double-click, delete/undo, editable edge relations, source-lost feedback, and source jump, but it still lacks the full toolbar, richer inspector polish, and i18n promised in specs | The team can overestimate MVP completeness and build on unstable assumptions | Use the traceability matrix as the implementation baseline; record partial coverage explicitly before marking slices done | Team | Active |
