...
| R-02 | Source jump can still land on the wrong message | `src/services/sourceLocator.ts` now uses role, dom id, occurrence index, and start/end previews, and runtime validation now proves one fallback path without DOM ids, but it still lacks a fully stable message identity | Clicking "Jump to source" may still scroll to the wrong chat block in repeated or edited conversations | Add source-lost icon/tooltip treatment, broader fallback fixtures, and stronger message identity strategies; verify against long and repeated chats | Runtime spine | Active |
...
| R-06 | Runtime test coverage is still narrow | `ci` now includes unpacked-extension runtime validation plus source-jump success and source-lost feedback coverage on the mock host, but automated coverage still focuses on a narrow DOM fixture plus generator, document merge, and store basics | Regressions in DOM parsing, observation, and source reveal can still slip through before live-ChatGPT validation | Extend tests to `chatAdapter`, repeated-source ambiguity, richer merge scenarios, and more DOM fixtures as the code is refactored for testability | Engineering | Active |
