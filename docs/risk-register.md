...
| R-06 | Runtime test coverage is still narrow | `ci` now includes unpacked-extension runtime validation plus source-jump success and a degraded source-lost path that keeps a wrong-assistant fallback from being accepted, but automated coverage still focuses on a narrow DOM fixture plus generator, document merge, and store basics | Regressions in DOM parsing, observation, and source reveal can still slip through before live-ChatGPT validation | Extend tests to `chatAdapter`, repeated-source ambiguity, richer merge scenarios, and more DOM fixtures as the code is refactored for testability | Engineering | Active |
